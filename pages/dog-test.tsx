...
import { DOG_QUESTIONS } from '../src/lib/dogQuestions'
import { calculateDogType } from '../src/lib/dogQuestions'

export default function DogTest() {
  ...
  const handleAnswer = (optionKey: string) => {
    const question = DOG_QUESTIONS[currentQuestion]
    setAnswers(prev => ({
      ...prev,
  ...
  const handleNext = () => {
    ...
    setError(null)
    if (currentQuestion < DOG_QUESTIONS.length - 1) {
  ...
  const handleSubmit = () => {
    const allAnswered = DOG_QUESTIONS.every(q => answers[q.id])
    if (!allAnswered) {
      setError('请回答所有问题')
      return
    }
  ...
  const progress = ((currentQuestion + 1) / DOG_QUESTIONS.length) * 100
  const question = DOG_QUESTIONS[currentQuestion]
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg hover:from-blue-600 hover:to-cyan-600 transition-all transform hover:scale-105 shadow-lg text-lg font-medium"
            >
              开始测试
            </button>
          </div>
  ...
  return (
    <>
      <Head>
        <title>你是什么狗狗测试 - LY Analytics</title>
      </Head>
      <div className="min-h-screen py-8 px-4 bg-gradient-to-br from-blue-50 to-cyan-50">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            ...
            <div className="text-sm text-gray-600 mb-4">
              问题 {currentQuestion + 1} / {DOG_QUESTIONS.length}
            </div>
          ...
          <div className="flex justify-between gap-4">
            ...
            {currentQuestion === DOG_QUESTIONS.length - 1 ? (
              <button
