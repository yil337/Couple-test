...
import { PIG_QUESTIONS } from '../src/lib/pigQuestions'
import { calculatePigType } from '../src/lib/pigQuestions'

export default function PigTest() {
  ...
  const handleAnswer = (optionKey: string) => {
    const question = PIG_QUESTIONS[currentQuestion]
    setAnswers(prev => ({
      ...prev,
  ...
  const handleNext = () => {
    ...
    setError(null)
    if (currentQuestion < PIG_QUESTIONS.length - 1) {
  ...
  const handleSubmit = () => {
    const allAnswered = PIG_QUESTIONS.every(q => answers[q.id])
    if (!allAnswered) {
      setError('请回答所有问题')
      return
    }
    ...
    const pigType = calculatePigType(answers)
  ...
  const progress = ((currentQuestion + 1) / PIG_QUESTIONS.length) * 100
  const question = PIG_QUESTIONS[currentQuestion]
  ...
  return (
    <>
      <Head>
        <title>你是什么猪猪测试 - LY Analytics</title>
      </Head>
      <div className="min-h-screen py-8 px-4 bg-gradient-to-br from-pink-50 to-purple-50">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            ...
            <div className="text-sm text-gray-600 mb-4">
              问题 {currentQuestion + 1} / {PIG_QUESTIONS.length}
            </div>
          ...
          <div className="flex justify-between gap-4">
            ...
            {currentQuestion === PIG_QUESTIONS.length - 1 ? (
              <button
